"""
Logic Layer implementation from LogiX.
Adapted from: https://github.com/spideralessio/logix

This module provides interpretable logic-based layers that can replace
standard MLPs for classification tasks.
"""
...
class Phi(nn.Module):
    """
    Input transformation layer that applies learned sigmoid transformations.
    
    Parameters
    ----------
    features : int
        Number of input features
    calculate_entropy : bool, default=True
        Whether to calculate entropy for regularization
    """
...
class DummyPhi(nn.Module):
    """Identity transformation (no learned parameters)."""
...
class LogicalLayer(nn.Module):
    """
    Interpretable logic-based layer.
    
    This layer learns logical rules using weighted combinations of input
    features with sigmoid activations. It provides interpretability through
    extractable logic rules.
    ...
    dummy_phi_in : bool, default=False
        If True, use identity input transformation
    use_weight_sigma : bool, default=True
        If True, learn sigmoid weights
    use_weight_exp : bool, default=True
        If True, learn exponential weights
        
    Attributes
    ----------
    reg_loss : float
        Regularization loss for sparsity
    entropy_loss : float
        Entropy loss for uncertainty quantification
    """
    def __init__(self, in_features, out_features, dummy_phi_in=False,
                 use_weight_sigma=True, use_weight_exp=True):
        super().__init__()
        ...
        if use_weight_exp and use_weight_sigma:
            ...
            self.weight_exp = nn.Parameter(
                torch.Tensor(out_features, in_features)
            )
        else:
            self.weight_exp = torch.zeros((out_features, 1)).float()
                
        self.b = nn.Parameter(torch.Tensor(out_features))
        self.prune_ = nn.Parameter(
    ...
    @staticmethod
    @torch.no_grad()
    def find_logic_rules(w, t_in, t_out, max_rule_len=float('inf'),
                        max_rules=float('inf')):
        """
        Extract interpretable logic rules from the learned weights.
        ...
        t_out : float
            Output threshold
        max_rule_len : int, default=inf
            Maximum rule length
        max_rules : int, default=inf
            Maximum number of rules
            
        Returns
        -------
        set
            Set of logic rules as tuples of feature indices
        """
        w = w.clone()
        t_in = t_in.clone()
        t_out = t_out.clone()
        ...
        def find_logic_rules_recursive(index, current_sum):
            ...
            for i in range(index, idxs_to_visit.shape[0]):
                ...
                find_logic_rules_recursive(
                    i + 1,
                    current_sum + w[idxs_to_visit[i]]
                )
    ...
    def extract_rules(self):
        """
        Extract logic rules for all output neurons.
        
        Returns
        -------
        list
            List of sets, one per output neuron, containing logic rules
        """
        ws = self.weight
        t_in = self.phi_in.t
        t_out = -self.b
