#!/usr/bin/env python3
"""
Train Next Cow Prediction with One-Hot Encoding (no embeddings).

One-hot encoding treats cow IDs as true categorical features,
which may be more interpretable for logic layers.

Usage:
    python train_mlp_onehot.py --dataset-dir next_cow_data
"""
...
class NextCowDataset(Dataset):
    """PyTorch Dataset for Next Cow Prediction with one-hot encoding."""
    
    def __init__(self, sequences, num_cows):
        """
        Args:
            sequences: List of sequence dicts from pickle file
            num_cows: Total number of unique cows (vocabulary size)
        """
    ...
    def __getitem__(self, idx):
        ...
        # Shape: (seq_len, num_cows)
        input_seq_ids = seq['input_sequence_ids']
        input_onehot = torch.zeros(len(input_seq_ids), self.num_cows)
        for i, cow_id in enumerate(input_seq_ids):
            input_onehot[i, cow_id] = 1.0
        
        return {
            'source_cow_id': torch.tensor(seq['source_cow_id'], dtype=torch.long),
            'input_sequence': input_onehot,  # (seq_len, num_cows) one-hot
            'target': torch.tensor(seq['target_id'], dtype=torch.long)
        }


class NextCowOneHotModel(nn.Module):
    """MLP model with one-hot encoded inputs (no embeddings)."""
    
    def __init__(self, num_cows, hidden_dims=[256, 256], 
                 dropout=0.3, use_logic_layer=False, logic_dim=128):
        super().__init__()
        ...
        # Input dimension: seq_length * num_cows (flattened one-hot)
        input_dim = self.seq_length * num_cows
    ...
    def forward(self, source_cow_id, input_sequence):
        ...
        Args:
            ...
            input_sequence: (batch_size, seq_len, num_cows) - one-hot encoded
        ...
        # Flatten one-hot sequence
        x = input_sequence.view(batch_size, -1)  # (batch_size, seq_len * num_cows)
...
def main():
    parser = argparse.ArgumentParser(
        description='Train Next Cow Prediction with One-Hot Encoding'
    )
    parser.add_argument(
        '--dataset-dir',
        type=str,
    ...
    parser.add_argument(
        '--output-dir',
        type=str,
        default='onehot_mlp_output',
        help='Output directory (default: onehot_mlp_output)'
    )
    
    args = parser.parse_args()
    
    print("=" * 70)
    print("NEXT COW PREDICTION - ONE-HOT ENCODING")
    print("=" * 70)
    ...
    train_dataset = NextCowDataset(train_sequences, vocab_size)
    val_dataset = NextCowDataset(val_sequences, vocab_size)
    ...
    model_type = "One-Hot + Logic Layer" if args.use_logic else "One-Hot + MLP"
    print(f"Model type: {model_type}")
    print(f"Hidden layers: {args.hidden_dims}")
    print(f"Input features: {9 * vocab_size} (9 timesteps × {vocab_size} one-hot)")
    
    model = NextCowOneHotModel(
        num_cows=vocab_size,
    ...
    if args.use_logic:
        ...
        try:
            ...
            # With one-hot encoding, rules directly reference input features
            print(f"\n   Note: With one-hot encoding, each feature represents")
            print(f"   a specific (timestep, cow_id) pair in the sequence")
